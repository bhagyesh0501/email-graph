...
Vtiger.Class("Vtiger_Detail_Js", {
	...
	*/
	...
	contactgetdata: function () {
		var self = this;
		jQuery('#getdata').live('click', function (e) {
			var element = jQuery(e.currentTarget);
			var record = self.getRecordId();
			var moduleName = app.getModuleName();
			window.location.replace("index.php?module="+moduleName+"&view=GetInfoAjax&record="+record+"&mode=recordinfo&app=NEW");
			app.request.post({ data: postData }).then(
				function (err, response) {
					// location.reload();
				}
